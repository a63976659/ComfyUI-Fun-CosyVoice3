...
基于阿里通义百聆语音大模型Fun-CosyVoice 3.0模型，为您提供极速、低显存占用且支持零样本克隆与方言控制的高质量语音合成节点
...
4. **方言**：18种以上的中文方言/口音（广东话、闽南话、四川话、东北话、陕西话、山西话、上海话、天津话、山东话、宁夏话、甘肃话等）可选择。
...
* **原因**：依赖未正确安装。
* **解决**：请重新执行“安装方法”中的第二步，确保 `git+...` 那条命令执行成功。
...
2. 模型下载后会缓存在 `ComfyUI/models/CosyVoice` 目录下，首次运行成功后，下次即可离线使用。
